...
function main() {
  ...
  gameOver
  let pacmanInterval, blinkyInterval
    ...
    setTimeout(() => {
      ...
      const checkGameInterval = setInterval(() => {
        ...
        if (cells.every(cell => !cell.classList.contains('seed') && !cell.classList.contains('biscuit'))) {
          ...
          clearInterval(checkGameInterval), clearInterval(blinkyInterval)
          pacman.ableToMove = false
          endGame()
    
        } else if ([blinky.position].includes(pacman.position) && pacman.flee) {
          ...
          if (pacman.lives <= 1) {
            ...
            clearInterval(pacmanInterval)
             resetGhost(blinky, 115, 'blinky', blinkyInterval)
